...
static __INLINE uint8_t wireless_reg_read(uint8_t reg)
{
	...
	wireless_CSN_low();
	spiTx(RF_SPI_BASE, send, 2, receive);
	wireless_CSN_high();
...
static __INLINE void wireless_reg_write(uint8_t reg, uint8_t data)
{
	uint8_t receive[2];
	uint8_t send[2];
	send[0] = (reg & 0x1F) | NRF24L01_CMD_W_REGISTER;
	send[1] = data;
...
static __INLINE void wireless_set_tx_addr(uint8_t  *tx_addr)
{
	...
	uint8_t receive[6];
	uint8_t send[6];
	send[0] = (NRF24L01_TX_ADDR_R | NRF24L01_CMD_W_REGISTER);
	...
	wireless_CSN_low();
	spiTx(RF_SPI_BASE, send, 6, receive);
	wireless_CSN_high();
}
...
static __INLINE void wireless_tx_data_payload( uint32_t data)
{
	...
	uint8_t receive[5];
	uint8_t send[5];
	send[0] = NRF24L01_CMD_W_TX_PAYLOAD;
	
	for(i = 1; i < 5; i++){
		send[i] = (uint8_t) (data >> ((4-i)*8));	
	}
	
	wireless_CSN_low();
	spiTx(RF_SPI_BASE, send, 5, receive);
	wireless_CSN_high();
}
...
static __INLINE void wireless_rx_data_payload( uint32_t *data)
{
	...
	uint8_t receive[5];
	uint8_t send[5];
	send[0] = NRF24L01_CMD_R_RX_PAYLOAD;
	
	wireless_CSN_low();
	spiTx(RF_SPI_BASE, send, 5, receive);
	
	for(i = 1; i < 5; i++){
		*(data + (8*(4-i))) = receive[i]; 
	}
	wireless_CSN_high();
}
