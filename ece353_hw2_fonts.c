	...
	char character)
{
	...
	int16_t lut_index, i;	//overflow if not 16 bits
	int8_t j;
	
  partial_data = 1;

	//calculate the look up table index for the char
	lut_index = (character - 0x20) * 32;	
	
	//set the position of the character
	lcd_set_pos(X_pixel,X_pixel+FONT_WIDTH-1,Y_pixel-FONT_HEIGHT,Y_pixel - 1);

	//work backwards through each byte of the pixel map
	for (i = lut_index + 31; i >= lut_index; i--) {	//gets each subsequent byte of the bitmap
		byte = courierNewBitmap[i];
		mask = 0x01;
		
		//check each pixel for fg or bg
		for (j = 0; j < 8; j++) {
		  if (partial_data && (j < 5))	//skip extra bits
				continue;
			if ( (mask << j) & byte )	//isolate each bit
				lcd_write_data_u16(fg_color);
			else
				lcd_write_data_u16(bg_color);
		}
		
		//toggle whether or not there are extra bits in the byte
		partial_data = partial_data ^ 0x01;
	}
}
...
{
	uint16_t i, rw, column;
  column = X;
  rw = Y + 1;
	i = 0;

	while (*(msg + i)) {	//continue while we havn't hit NULL
    //check if column or row is too big
		if (column > CHAR_COLUMNS-1) {
      column = X;
			rw++;
			//dont want a space to be the first thing on the next column
			if ((*(msg + i)) == ' ') {;
				i++;
				continue;
			}
		}
		//reset to top if past the last row
    if (rw > CHAR_ROWS)
			rw = 1;
		
		//print char and setup for next one
		lcd_print_character(column * FONT_WIDTH, rw * FONT_HEIGHT, fg_color, bg_color, *(msg+i));
		column++;
		i++;
	}
