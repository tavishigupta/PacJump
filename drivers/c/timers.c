...
#define PRESCALAR_TEN  7
#define PRESCALAR_TWENTY  15
#define TEN_MS  62500	//this is the scaled values for the timer load regs
#define TWENTY_MS  62500
#define ONE_TENTH 5000000
...
bool gp_timer_config_32(uint32_t base_addr, uint32_t mode, bool count_up, bool enable_interrupts)
{
  ...
  gp_timer->CTL &= ~(TIMER_CTL_TAEN | TIMER_CTL_TBEN);
	...
	gp_timer->TAMR &= 0x0;
	gp_timer->TAMR |= mode;
	gp_timer->TAMR |= (count_up << 4);
	gp_timer->TAILR = 15000000; //.3 seconds, arbitrary, but it is calibrated with our speed and multiplier
	gp_timer->ICR |= TIMER_ICR_TATOCINT;
...
//  base_address          The base address of a general purpose timer
//
//  modeA                  bit mask for Periodic, One-Shot, or Capture for timer A
//
//  modeB                  bit mask for Periodic, One-Shot, or Capture for timer B
//
//  count_up              When true, the timer counts up.  When false, it counts
//                        down
//
...
//*****************************************************************************
bool gp_timer_config_start_16(uint32_t base_addr, uint32_t modeA, uint32_t modeB, bool count_up, bool enable_interrupts)
{
  uint32_t timer_rcgc_mask;
  uint32_t timer_pr_mask;
  TIMER0_Type *gp_timer;
  ...
  gp_timer = (TIMER0_Type *)base_addr;
	...
	//disable timers
  gp_timer->CTL &= ~(TIMER_CTL_TAEN | TIMER_CTL_TBEN); 
	// configure for 2 16 bit timers
	gp_timer->CFG = TIMER_CFG_16_BIT;
   
	//clear, then set timerA's mode, direction, load register value, and prescalar
	gp_timer->TAMR &= 0x0;
	gp_timer->TAMR |= modeA;
	gp_timer->TAMR |= (count_up << 4);	
	gp_timer->TAILR = TEN_MS;
	gp_timer->TAPR = PRESCALAR_TEN;
	
  //clear, then set timerB's mode, direction, load register value, and prescalar		
	gp_timer->TBMR &= 0x0;
	gp_timer->TBMR|= modeB;
	gp_timer->TBMR |= (count_up << 4);	
	gp_timer->TBILR = TWENTY_MS;
	gp_timer->TBPR = PRESCALAR_TWENTY;
	...
	if (enable_interrupts) {
	  gp_timer->IMR = TIMER_IMR_TATOIM | TIMER_IMR_TBTOIM;  //enable the MIS register
		
		//We are prioritizing the timers to be less important than the ADC conversion interupt		
		//set the priority and enable interrupt in the NVIC
		NVIC_SetPriority(TIMER0A_IRQn, 1);
		NVIC_EnableIRQ(TIMER0A_IRQn);
